    ...
    function View() {
        ...
        this.create = function (arr,i) {
            if (arr[i] instanceof Model) {
                ...
                if (arr[i].status) {
                    result += "<input type='checkbox' onchange='Controller.perform(Storage,"+i+");'><span class='input-group-addon-text'>" + arr[i].text + "</span></span></div></div></div>"
                } else {
                    result += "<input type='checkbox' onchange='Controller.perform(Storage,"+i+");' checked><span class='input-group-addon-text input-group-addon-text--finished'>" + arr[i].text + "</span></span></div></div></div>";
                }
                return result;
            } else {
